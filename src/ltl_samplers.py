...
def getLTLSampler(sampler_id, propositions):
    """
    The LTLSampler factory method that instantiates the proper sampler based on the `sampler_id`.
    """
    tokens = ["Default"]
    if (sampler_id != None):
...
class UntilTaskSampler(LTLSampler):
    ...
    def sample(self):
        ...
        for i in range(n_conjs):
            ...
            ltl = until_task if ltl is None else ('and',until_task,ltl)
        return ltl
...
class EventuallySampler(LTLSampler):
    ...
    e.g. in (eventually (a and eventually c)) and (eventually b), the two sequence tasks are "a->c" and "b".
    """
    ...
    def sample_sequence(self):
        ...
        while len(seq) < length:
            ...
            c = random.sample(population, 2) if random.random() < 0.25 else random.sample(population, 1)
            seq.append(c)
            last = c
